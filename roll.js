				...
				inputs += "<table class='dsa_attributes'>" + 
					...
					for(var j = 0; j < items.length; j++) {
						...
						for(var i = 0; i < category[1].length; i++) {
							...
							inputs += "<tr class='skill tree_" + category[0] + "'><td>" + item[0] + ":</td><td><input type='number' id='in_" + item[0].toLowerCase() + "' value='0'/></td><td>" + item[1] + "</td><td><img class='skill_dice' id='skill_dice_" + item[0].toLowerCase() + "' src='" + dice20logo + "' dice-name='" + item[0] + "' dice-attr='" + item[1].toLowerCase() + "'/></td></tr>";
						}
					}
					
					
				inputs += "</table>";
				$(".app-sidebar-tabs__content.app-sidebar-tabs__content--multiple").append("<section data-v-02f177dc='' data-v-5580c196='' id='tab-dsa' aria-labelledby='DSA' tabindex='0' role='tabpanel' style='display:none'>" + inputs + "</section>");
